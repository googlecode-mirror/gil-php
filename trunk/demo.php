...
GilDBW::insert('test',array('name'=>'hi','ext'=>'1'));//插入一条数据
//GilDBW::delete('test',array('id'=>'5'));
GilDBW::save(true);//执行所有修改,true表示，这是一个事务
...
GilCache::set('1', 'nihao', '123');//这里示范了key=>value式的缓存写入，默认使用了File引擎缓存，请在gilFramework下建立一个tmp文件夹用于存放临时文件
echo GilCache::get('1');

GilSession::set('hello','hi');//设置会话信息
echo GilSession::get('hello');//获得会话信息
GilSession::del('hello');//删除会话信息
GilSession::clean();//完全清除本次会话
